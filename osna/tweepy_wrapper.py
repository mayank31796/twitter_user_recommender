...
def get_twitter():
    ...
    auth = tweepy.AppAuthHandler('JaxuEZ63V5VfvzraFmXCQI1CA', 'mZ7CvXOPCmxX6ZWVTOL4wruG6wbcwYima17oIViODeCqblTKTc') 
    api = tweepy.API(auth, wait_on_rate_limit=True, wait_on_rate_limit_notify=True)
    print("Established Twitter Connection Successfully!")
    return auth, api

def get_friends(api, user,count=None):
    friends = []
    if count == None:
...
def get_deg2_users(api, friends, count):
    friendsd2 = set()
    backoff_counter = 1
    ct=0
    for f in friends: 
...
def save_json(file, path, name):
    with open(path+"{}.mm".format(name), 'w') as fp:
        json.dump(file, fp)
        
def save_pkl(file, path, name):
